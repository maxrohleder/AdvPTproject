...
class Protoss_status : public Race
{
  public:
    ...
    void boostEnd(string ID)
    {
        ...
        if (i == eventlist.end())
        {
            ...
            if (debug)
            {
                cout << "________________________CHRONO END________________________\n";
                cout << "there is no itemident produced by " << ID << " at time " << time << endl;
            }
        ...
        i->end_time = time + (int)floor(((double)(i->end_time - time) * 1.5));
        i->boosted = false;
        boostedbuildings.remove(ID);
  ...
  protected:
    ...
    void deactivateBuilding(string ID)
    {
        ...
        {
            if (debug)
                cout << "deactivateBuilding error protoss.h";
            exit(-1);
        }
        string id = *i;
    ...
    void distributeWorkers(int mins_needed, int vesp_needed, bool report_change)
    {
        ...
        if (debug)
            cout << "ideal ratio(vesp/total): " << vesp_to_total_ratio << "\n\t\tdelta_min: " << delta_min << " delta_vesp: " << delta_vesp;
        if (debug)
    ...
    bool stalkerBuild()
    {
        ...
        {
            if (gateway <= 0 || cybernetics_core <= 0)
                return false;
            --gateway; // occupie a place in gateway
            minerals -= 12500;
            vespene -= 5000;
    ...
    bool colossusBuild()
    {
        ...
        {
            if (robotics_facility <= 0 || robotics_bay <= 0)
                return false;
    ...
    bool hightemplarBuild()
    {
        ...
        {
            if (gateway <= 0 || templar_archives <= 0)
                return false;
            --gateway; // occupie a place in gateway
            minerals -= 5000;
            vespene -= 15000;
    ...
    bool mothershipBuild()
    {
        ...
        {
            if (nexus <= 0 || fleet_beacon <= 0 || mothership != 0)
                return false;
    ...
    void nexusFinish(string ID)
    {
        supply_max += 10;
        ++nexus;
        geyser_max += 2; // needed as assimilator build criterion
